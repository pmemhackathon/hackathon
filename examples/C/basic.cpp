...
static constexpr int NumaNode = 0;
  ...
  config
      ...
      .configureMemoryTiers(
          {MemoryTierCacheConfig::fromShm().setRatio(1).setMemBind(
              NumaBitMask().setBit(NumaNode))}) // allocate only from NUMA node 0
    ...
    for (size_t i = 0; i < NUM_ITEMS; ++i) {
      ...
      if (item) {
        ...
        assert(sp == value);
      	...
      	// verify node id
	    int numa_node = get_numa_node_by_ptr((void*) item->getMemory());
	    assert(numa_node == NumaNode);
      } else {
        ++nNotFound;
      }
    }
