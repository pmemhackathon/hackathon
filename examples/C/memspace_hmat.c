...
int main(void) {
    ...
    // Lets now allocate memory from the highest capacity node
    ret = createMemoryProvider(&hProvider, umfMemspaceHighestCapacityGet());
    if (ret != UMF_RESULT_SUCCESS) {
        return -1;
    }
    ...
    printf("Allocated memory at %p from the highest capacity node: %d\n", ptr,
           nodeId);
